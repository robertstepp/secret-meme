...
$types = mysqli_query($con,"SELECT `groupID` AS `QgroupID`, 'uid' FROM `bookmarks`.`bookmarks` AS `bookmarks` WHERE home='1' AND uid='" . $_SESSION['sess_user_id'] . "' GROUP BY `groupID` ORDER BY `QgroupID` ASC");
